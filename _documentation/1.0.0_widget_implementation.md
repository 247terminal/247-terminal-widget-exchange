...
interface NewsCardProps {
    ...
    on_trade_click?: (news_item: NewsItem) => void;
}
...
export function NewsCard({ news_item, sentiment, on_click, on_trade_click }: NewsCardProps) {
    const format_time = (timestamp: number): string => {
        const date = new Date(timestamp);
        return date.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    };

    const handle_trade_click = (e: Event) => {
        e.stopPropagation();
        on_trade_click?.(news_item);
    };

    return (
        <Card
            onClick={() => on_click(news_item._id)}
        ...
        >
            ...
            <Meta>
                {news_item.source && <Source>{news_item.source}</Source>}
                <Time>{format_time(news_item.time)}</Time>
                {sentiment && <SentimentBadge sentiment={sentiment.sentiment} />}
                {on_trade_click && (
                    <TradeButton onClick={handle_trade_click}>
                        Trade
                    </TradeButton>
                )}
            </Meta>
...
**`src/features/news_feed/sentiment_badge.tsx`**
```typescript
import { h } from 'preact';
import styled from 'styled-components';
...
const Badge = styled.span<{ sentiment: string }>`
    padding: 2px 8px;
    border-radius: 12px;
    font-size: 11px;
    font-weight: 600;
    ...
    background-color: ${({ theme, sentiment }) => {
        switch (sentiment) {
            ...
            case 'negative': return theme.colors.danger + '20';
...
export function SentimentBadge({ sentiment }: SentimentBadgeProps) {
    return <Badge sentiment={sentiment}>{sentiment}</Badge>;
}
```
